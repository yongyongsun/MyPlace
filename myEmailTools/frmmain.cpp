...
{
    ui->setupUi(this);
    qDebug()<<QString("我是中文").toStdString().c_str();
    MailClient cli;
    ui->txtContent->setPlainText(QString::fromLocal8Bit(cli.str.toLatin1().data()));
}
...
//    //构建邮件正文
//    MimeHtml text;
//    text.setHtml(ui->txtContent->toPlainText());
...
//    }
    ...
    //add new type
    smtp.setMailSender(ui->txtSenderAddr->text());

    QStringList receiver = ui->txtReceiverAddr->text().split(';');
    smtp.setAddMailRevicer(receiver);

    smtp.setMailTitle(ui->txtTitle->text());

    smtp.setMailContent(ui->txtContent->toPlainText());

    QString atta=ui->txtAtta->text();
    if (atta != "")
    {
        QStringList attas=atta.split(";");
        smtp.setAddAttachment(attas);
    }
